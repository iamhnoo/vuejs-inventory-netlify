<template>
  ...
  <!-- Backdrop -->
  <aside
    :class="{ hidden: this.$store.state.showSideMenu }"
    class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto bg-gray-100 dark:bg-gray-800 xl:hidden"
  >
    <div class="py-4 text-gray-500 dark:text-gray-400">
      <a class="ml-6 text-lg font-bold text-gray-800 dark:text-gray-200" href="#">Inventory</a>
      ...
      <ul class="mt-6">
        <li class="relative px-6 py-3">
          ...
          ></span>
          <a
            @click="onClickSideMenu('dashboard')"
      ...
      <ul>
        <li class="relative px-6 py-3">
          <a
            @click="onClickSideMenu('products')"
            class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
          >
            <svg
              class="w-5 h-5"
...
export default {
  methods: {
    onClickLogout() {
      ...
      // window.location.href = "/login" //เข้าสู่หน้า login ด้วยคำสั่ง javascript
    },
    onClickSideMenu(menudetail) {
      switch (menudetail) {
        case "dashboard":
