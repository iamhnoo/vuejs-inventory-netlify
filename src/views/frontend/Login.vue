        ...
        <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
          <div class="w-full">
            ...
            <form @submit.prevent="onSubmit">
              ...
              />
              <div v-if="v$.password.$error" class="text-sm text-red-500 mt2">
...
export default {
  ...
  methods: {
    submitForm() {
      this.v$.$validate();
      /** method .post has 2 parameter then call login api include data is email and password 
        if login success then axios will call .then method and receive response parameter 
        if login fail or has all error then axios will call .catch method and receive error parameter */
      if (!this.v$.$error) {
        http
          .post("login", {
            email: this.email,
          ...
          .then((response) => {
            ...
            const Toast = this.$swal.mixin({
              ...
              timer: 1500, // เวลาหน่วยเป็น milisec มีฐานเป็น 1000
            ...
            Toast.fire({
              ...
              /**
               * เรียกว่าการทำ callback ซึ่งจำเป็นที่ต้องนำ redirect มาใส่ในจังหวะนี้
               * เพราะการทำงานกับ web จะเป็นการทำงานแบบรวดเดียวจบ ไม่มีการหยุดรอ เหมือน desktop app
               * */
              // ส่งไปหน้า backend
              // this.$router.push("backend"); //คำสั่งของ vue แบบนี้ เรียกจาก url เป็นแบบเดียวกับการ update ID ทำให้ไม่มีการ load ทั้งหน้า มีผลทำให้ ไม่มีการ load ค่าจาก localStroage
              this.$router.push({ name: "Dashboard" });
              // window.location.href = '/backend' //เป็นการสั่งให้ refresh ทั้งหน้าของ backend
            });
          })
          .catch((error) => {
            if (error.response) {
              if (error.response.status == 401) {
                const Toast = this.$swal.mixin({
                  ...
                  position: "top-end",
                  showConfirmButton: false,
                  timer: 3000, // เวลาหน่วยเป็น milisec มีฐานเป็น 1000
                  timerProgressBar: true,
                  didOpen: (toast) => {
                    toast.addEventListener("mouseenter", this.$swal.stopTimer);
                    toast.addEventListener("mouseleave", this.$swal.resumeTimer);
  ...
  validations() {
    return {
      ...
      password: {
        required: helpers.withMessage("ต้องป้อนพาสเวิดก่อน", required),
